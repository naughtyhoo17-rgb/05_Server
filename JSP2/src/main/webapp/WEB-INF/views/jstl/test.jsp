...
- 작성법 2 : 작성된 조건식의 결과는 무조건 T/F
	...
	<c:set var="test3" value="금요일" /> <%-- 따로 지정 안했으니 page scope에 저장된 것 --%>
 ...
 * 주의 사항 *
바로 한단계 아래에는
c:when, c:otherwise, 공백, jsp용 주석(not HTML 주석)만 작성할 수 있고 나머지 모두 에러 발생!!
 ...
 => 아무 속성도 없음!
	...
	<c:choose>
		<%-- 
			...
			3) 나머지는 다 false
		 --%>
		<c:when test="${empty param.age}"> <%-- <c:when test="${param.age == null}"> --%>
			<h4>age값이 없습니다</h4>
		</c:when>
...
* varStatus에서 제공되는 값
 ...
 - count : 현재 몇 바퀴 째인지 반복 횟수 반환 (1부터 시작)
