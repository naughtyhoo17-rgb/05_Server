...
<body>
	...
	<pre>
Servlet/JSP에는 네 종류 범위를 나타낸 내장 객체가 존재
...
=> 서버에 접속한 클라이언트를 나타내거나, 관련 정보를 get/set 할 수 있는 객체
  (session 객체는 서버에서 관리)
=> session은 브라우저마다 하나 씩 생성!!
=> session의 범위 : 사이트 접속 ~ 브라우저 종료/세션 시간 만료
  thus, session이 유지되는 상태에서는 아무 곳에서나 session에 세팅된 값을 가져다 사용 가능!
  (ex. 로그인 정보)  	
  
  <h3>4. application (ServletContext)</h3>
=> 하나의 웹 애플리케이션마다 1 개만 생성되는 객체 (Server 키면 1개 생성)
=> application 범위 : 서버 실행 ~ 서버 종료
  thus, 세팅된 값 어디서든 사용 가능  
...
- 범위별 객체에 세팅된 값 얻어오는 EL
 1) \${000Scope.key}
 	=> ex) \${requestScope.key}	
 2) \${key}
 	=> 좁은 범위 객체부터 탐색하여 일치하는 key가 있으면 해당 scope에서 값을 얻어옴	
	</pre>
