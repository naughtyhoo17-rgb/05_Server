...
/* 클라이언트로부터 "/example1" 이라는 요청을 받아 처리한 후
  응답을 해 줄 서블릿 클래스 ExampleController1을 만든다. */
...
public class ExampleController1 extends HttpServlet{
	...
	/* doGet() 메서드
	  => GET 방식 요청을 처리하는 메서드
	  HttpServlet을 상속받으면 사용 가능, 오버라이딩하여 사용 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		/* HttpservletRequest : 
		 클라이언트 요청 시 생성되는 객체
		 요청 시 전달된 데이터, 요청한 클라이언트 정보, 유연한 요청 처리를 위한 객체 제공 */
		
		/* HttpServletResponse : 
		 클라이언트 요청 시 생성되는 객체
		 서버가 클라이언트에게 응답하기 위한 방법을 제공하는 객체
		 서버가 클라이언트 쪽으로 만들어진 페이지를 제공하기 위해서는 출력용 스트림(PrintWriter) 제공 */
		...
		/* 서버에서 클라이언트 쪽으로 응답 처리
		 
		  1) 서버에서 HTML 코드를 만들어 클라이언트에게 전달 == 응답(Response)
		 
		  2) 응답받은 내용을 클라이언트의 브라우저가 해석하여 화면에 출력(렌더링) */
		...
		resp.setContentType("text/html; charset=UTF-8");  
		 ...
		 // 응답처리 2 : 서버에서 클라이언트로 연결되는 출력용 스트림 얻어오기
		...
		sb.append("</html>");
		 ...
		 // 응답처리 4 : 출력스트림 이용해 클라이언트에게 HTML 코드 제공(응답)
